...
# ============================================================================
# INTENT PRE-CLASSIFICATION ENDPOINT
# Fast binary classification: action vs conversational
...
class ClassifyIntentResponse(BaseModel):
    intent: str  # "action" or "conversational"


# Minimal prompt for fast classification (~50 tokens)
INTENT_PRECLASSIFY_PROMPT = """Classify this voice command as "action" or "conversational".

- action: User wants to DO something in the app (navigate, create, delete, select, click, start, stop, enroll, switch tabs, go live, upload, submit, etc.)
- conversational: User is asking a question, greeting, thanking, or having open conversation

Input: "{transcript}"
Output (just one word, action or conversational):"""


@router.post("/classify-intent", response_model=ClassifyIntentResponse)
async def classify_voice_intent(request: ClassifyIntentRequest):
    """
    Fast binary intent classification for voice commands.

    Used by frontend to decide:
    - action: Mute ElevenLabs agent, route to backend for UI control
    - conversational: Let ElevenLabs respond directly (faster, better quality)

    Uses minimal LLM prompt for speed (~200-300ms).
    """
    ...
    try:
        from workflows.llm_utils import get_fast_voice_llm, invoke_llm_with_metrics
