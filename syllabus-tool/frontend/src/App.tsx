...
function App() {
  ...
  // Listen for session-expired events from fetchWithAuth
  useEffect(() => {
  ...
  const handleTemplateFillReset = () => {
    ...
    setTemplateCourseInfo(null);
    setRegeneratingSection(null);
    setStep('upload');
  };

  // --- Template Fill: Regenerate Section ---
  const handleRegenerateSection = async (sectionId: string) => {
    if (!templateFileId || !templateCourseInfo) return;
    const section = templateSections.find(s => s.id === sectionId);
    if (!section) return;

    const instruction = window.prompt('Optional instruction (leave empty for a fresh regeneration):') ?? '';

    setRegeneratingSection(sectionId);
    try {
      const response = await fetchWithAuth(`${apiUrl}/generate/fill-template/regenerate-section`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          reference_file_id: templateFileId,
          section_id: sectionId,
          paragraph_indices: section.paragraphIndices,
          course_title: templateCourseInfo.title,
          target_audience: templateCourseInfo.audience,
          duration: templateCourseInfo.duration,
          language: templateCourseInfo.language,
          instruction,
        }),
      });
      ...
      setTemplateSections(prev =>
        prev.map(s => s.id === sectionId ? { ...s, filledText: result.filled_text } : s)
      );
    } catch (error) {
